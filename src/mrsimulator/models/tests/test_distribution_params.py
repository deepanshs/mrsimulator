...
def test_dist_1():
    ...
    assert params[f"{prefix}_0_iso_shift"] == 23.4
    assert params[f"{prefix}_0_abundance"] == 1.0

    params[f"{prefix}_0_sigma"].value = 10
    params[f"{prefix}_0_iso_shift"].value = -63.4
...
def test_dist_2():
    ...
    prefix = "extended_czjzek"

    params = Parameters()
    params = dist.add_lmfit_params(params=params, i=0)
    assert len(params.keys()) == 5

    assert params[f"{prefix}_0_zeta"] == 4.1
    assert params[f"{prefix}_0_eta"] == 0.2
    assert params[f"{prefix}_0_epsilon"] == 0.3
    assert params[f"{prefix}_0_iso_shift"] == 0.0
    assert params[f"{prefix}_0_abundance"] == 1.0

    params[f"{prefix}_0_zeta"].value = -30.2
    params[f"{prefix}_0_eta"].value = 0.65
    params[f"{prefix}_0_epsilon"].value = 0.6
    params[f"{prefix}_0_iso_shift"].value = -3.4
