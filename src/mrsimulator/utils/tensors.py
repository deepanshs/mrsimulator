...
def mehring_parameters_to_tensor(euler_angles: List[float], 
...
def tensor_to_haeberlen_parameters(tensor: np.ndarray) -> Tuple[np.ndarray, float, float, float]:
    tensor = (tensor + tensor.T) / 2    # Make sure the tensor is symmetric
    ...
    idx = np.argsort(np.abs(eigenvalues)) # Sort the eigenvalues by their absolute values
...
def haeberlen_parameters_to_tensor(euler_angles: List[float], 
                                   zeta_sigma: float, 
                                   eta_sigma: float, 
                                   isotropic_component: float) -> np.ndarray:
    eigenvalues = np.array([-zeta_sigma * (1 + eta_sigma)/2,
...
def mehring_principal_components_to_maryland(lambdas: List[float]) -> Tuple[float, float, float]:
    isotropic = np.mean(lambdas)
    span = lambdas[2] - lambdas[0]
    skew = 3 * (isotropic - lambdas[1]) / span
    return span, skew, isotropic

def maryland_to_mehring_principal_components(isotropic: float, span: float, skew: float) -> np.ndarray:
...
def maryland_to_haeberlen_parameters(isotropic: float, span: float, skew: float) -> Tuple[float, float, float]:
    lambdas = maryland_to_mehring_principal_components(isotropic, span, skew)
    euler_angles, zeta_sigma, eta_sigma, isotropic_component = tensor_to_haeberlen_parameters(lambdas)
    return zeta_sigma, eta_sigma, isotropic_component

def haeberlen_parameters_to_maryland(zeta_sigma: float, eta_sigma: float, isotropic_component: float) -> Tuple[float, float, float]:
    tensor = haeberlen_parameters_to_tensor(np.array([0, 0, 0]), zeta_sigma, eta_sigma, isotropic_component)
    return mehring_principal_components_to_maryland(tensor_to_mehring_parameters(tensor))
