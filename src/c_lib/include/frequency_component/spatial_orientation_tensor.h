 ...
 * perturbation expansion of the nuclear shielding Hamiltonian, in the principal
 * axis system (PAS), include contributions from the zeroth and second-rank
 ...
 * rotation of @f$\varsigma_{2,n}^{(\sigma)}@f$ is applied following,
 * @f[ \mathcal{R'}_{2,n}^{(\sigma)}(\Theta) =
 *                                \sum_{m = -2}^2 D^2_{m, n}(\Theta)
 *                                \varsigma_{2,n}^{(\sigma)},
 * @f]
 * where @f$\mathcal{R'}_{2,n}^{(\sigma)}(\Theta)@f$ are the tensor components
 * in the frame defined by the Euler angles @f$\Theta@f$.
 *
 * @note
 ...
 *    frequency**.
 *
 * @param R_0 A pointer to an array of length 1, where the components of the
 *      zeroth-rank irreducible tensor,
 *      @f$\mathcal{R'}_{0,0}^{(\sigma)}(\Theta)/2\pi@f$, is stored.
 *
 * @param R_2 A pointer to a complex array of length 5, where the components of
 *      the second-rank irreducible tensor,
 ...
 * principal axis system (PAS), include contributions from the second-rank
 * irreducible tensor which follow,
 * @f[ \left.
 *      \begin{aligned}
 ...
 * a Wigner rotation of @f$\varsigma_{2,n}^{(q)}@f$ is applied following,
 * @f[ \mathcal{R'}_{2,n}^{(q)}(\Theta) = \sum_{m = -2}^2 D^2_{m, n}(\Theta)
 *                                            \varsigma_{2,n}^{(q)}.
 * @f]
 * where @f$\mathcal{R'}_{2,n}^{(q)}(\Theta)@f$ are the tensor components in the
 ...
 * @param R_2 A pointer to a complex array of length 5, where the components of
 *      the second-rank irreducible tensor,
 *      @f$\mathcal{R'}_{2,n}^{(q)}(\Theta)/2\pi@f$, is stored ordered as
 ...
 * perturbation expansion of the electric quadrupole Hamiltonian, in the
 * principal axis system (PAS), include contributions from the zeroth, second,
 * and fourth-rank irreducible tensors which follow,
 * @f[\left.
 ...
 * @f$\varsigma_{4,n}^{(qq)}@f$ are applied following,
 * @f[
 ...
 * components in the frame defined by the Euler angles @f$\Theta@f$,
 * respectively.
 *
 * @note
 *  - When @f$\Theta = [0,0,0]@f$,
...
/**
 * The scaled spatial orientation tensors (sSOT) from the first-order
 * perturbation expansion of the @f$J@f$-coupling Hamiltonian under
 * weak-coupling limit, in the principal axis system (PAS), include
 * contributions from the zeroth and second-rank irreducible tensors which
 * follow,
 * @f[ \left.
 ...
 *     \right\} \text{Rank-2},
 * @f]
 * where @f$J_\text{iso}@f$ is the isotropic @f$J@f$-coupling, and
 ...
 * @note
 *  - When @f$\Theta = [0,0,0]@f$,
 *    @f$\mathcal{R'}_{2,n}^{(J)}(\Theta) = \varsigma_{2,n}^{(J)}@f$ where
 *    @f$ n \in [-2,2]@f$.
 ...
 *
 * @param R_0 A pointer to an array of length 1, where the zeroth-rank
 *    irreducible tensor, @f$\mathcal{R'}_{0,0}^{(J)}(\Theta)/2\pi@f$, is
 *    stored.
 *
 * @param R_2 A pointer to a complex array of length 5, where the second-rank
 *      irreducible tensor, @f$\mathcal{R'}_{2,n}^{(J)}(\Theta)/2\pi@f$,
 *      is stored ordered as
 *      @f$\left[\mathcal{R'}_{2,n}^{(J)}(\Theta)/2\pi\right]_{n=-2}^2@f$.
 *
