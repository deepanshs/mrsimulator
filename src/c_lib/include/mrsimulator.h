...
void MRS_plan_update_from_sample_rotation_frequency_in_Hz(MRS_plan *plan, double increment,
 ...
 *            event.
 */
void MRS_get_normalized_frequencies_from_plan(MRS_averaging_scheme *scheme, MRS_plan *plan,
                                              double R0, complex128 *R2, complex128 *R4,
                                              bool refresh, MRS_dimension *dim, double fraction);

void MRS_get_frequencies_from_plan(MRS_averaging_scheme *scheme, MRS_plan *plan, double R0,
                                   complex128 *R2, complex128 *R4, bool refresh,
                                   MRS_dimension *dim);
 ...
 * @param transition A pointer to the spin quantum numbers from the inital and final states of the
 * 				spin transition packed as initial quantum numbers followed by the final quantum numbers.
 * @param allow_fourth_rank A boolean, if true, evalutes the frequency contributions from the
 * 				fourth-rank tensor.
 * @param R0 A pointer to an array where the frequency contribution from the zeroth-rank tensor is
 * 				stored.
 * @param R2 A pointer to a complex array where the frequency contributions from the second-rank
 * 				tensor are stored.
 * @param R4 A pointer to a complex array where the frequency contributions from the fourth-rank
 * 				tensor are stored.
 * @param R0_temp A pointer to an array where the frequency contribution from the zeroth-rank tensor
 * 				is temporarily stored.
 * @param R2_temp A pointer to a complex array where the frequency contributions from the
 * 				second-rank tensor are temporarily stored.
 * @param R4_temp A pointer to a complex array where the frequency contributions from the
 * 				fourth-rank tensor are temporarily stored.
 * @param B0_in_T The magnetic flux density of the macroscopic external magnetic field in T.
 * @param freq_contrib A pointer to a stack of boolean frequency contribs repeated number_of_events
 * 				times. The order of the freq contribs follow
