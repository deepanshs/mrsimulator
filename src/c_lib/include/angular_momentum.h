...
/**
 * @brief Evaluates @f$d^{l}_{m_1, m_2}(\beta)@f$ wigner-d element of the given angle @f$\beta@f$.
 *
 ...
 */
extern double wigner_d_element(const int l, const int m1, const int m2, const double beta);

/**
 * @brief Evaluates @f$n@f$ wigner-d matrices of rank @f$l@f$ at @f$n@f$
 *          angles given in radians.
 *
 * At a given angle, @f$\beta@f$, the wigner-d matrix, @f$d^{l}\left(m_1, m_2 | \beta\right)@f$, is
 * a @f$(2l+1) \times (2l+1)@f$ square matrix where
 ...
 * @param n The number of wigner-d matrix to evaluate. This is also the length of array, `beta`.
 * @param beta Pointer to array of length @f$n@f$, where the @f$\beta@f$ are stored in radians.
 * @param wigner A pointer to an array of length `n*(2*l+1)*(2*l+1)`.
 */
extern void wigner_d_matrices(const int l, const int n, const double *beta, double *wigner);

// Specialized methods ..................................................... //

/**
 * @brief Evaluates @f$d^{l}_{m_1, m_2}(\beta)@f$ wigner-d element from @f$\exp(i\beta)@f$ of the
 *      angle @f$\beta@f$.
 *
 ...
 * @param m1 The quantum number @f$m_1@f$.
 * @param m2 The quantum number @f$m_2@f$.
 * @param exp_I_beta A pointer to an array of size two, where cosine and sine of the angle
 *      @f$\beta@f$ stored.
 * @return The wigner-d element, @f$d^{l}_{m_1, m_2}(\beta)@f$.
 */
 ...
 * @param exp_I_beta A pointer to an array of length `2*n`, where the cosine and sine of the angles
 *      are stored.
 * @param wigner A pointer to an array of length `n*(2*l+1)*(2*l+1)`.
 */
extern void wigner_d_matrices_from_exp_I_beta(const int l, const int n,
                                              const void *restrict exp_I_beta, double *wigner);
 ...
 * @param wigner A pointer to nx(2l+1)x(2l+1) wigner-d matrices of rank @p l. The wigner matrices
 *          are stacked in a row major order.
 ...
 * @param R_out A pointer to a 1D-array of final vectors after rotation. The length of this vector
 *          is `n*(2*l+1)`, where the vector of index `i*(2*l+1)` is rotated with the wigner-lj
 *          matrix at index `i*(2*l+1)*(2*l+1)`.
 */
 ...
 * @param R_in A pointer to the array of coefficients from the l rank tensors of length 2xl+1 before
 *          rotation.
 * @param R_out A pointer to the array of coefficients from the l rank tensors of length 2xl+1 after
 *          rotation.
 */
extern void single_wigner_rotation(const int l, const double *euler_angles, const void *R_in,
                                   void *R_out);
 ...
 * @param exp_Im_alpha A pointer to a `4 x octant_orientations` array with the exp(-Im α) with
 *          `octant_orientations` as the leading dimension and ordered as m=[-4,-3,-2,-1].
 * @param w2 A pointer to a stack of second rank tensor coefficients after rotation with second rank
 *          wigner matrices. The length of w2 is `octant_orientations x n_octants x 5` with 5 as the
 *          leading dimension.
 * @param w4 A pointer to a stack of fourth rank tensor coefficients after rotation with fourth rank
 *          wigner matrices. The length of w4 is `octant_orientations x n_octants x 9` with 9 as the
 *          leading dimension.
 */
