 ...
 *                                  Site structure
 * =================================================================================
 * Coupling structure is a collection of couplings.
 **/
struct __site_struct {
   ...
   * parameters (eta) of each site within a spin system. */
  double *shielding_symmetric_eta;

  /* Pointer to an array of symmetric nuclear shielding tensor Euler angles
     in radians, packed as alpha, beta, and gamma. The array size is
  ...
  /* Pointer to an array of quadrupole coupling constants in Hz of each site
   * within a spin system. If the site is not a quadrupole, a value of zero is
   * set instead. */
  ...
  /* Pointer to an array of symmetric electric field gradient tensor Euler
     angles in radians, packed as alpha, beta, and gamma. The array size is
     3*number_of_sites. The Euler angle rotate the tensor from the principal
     axis system (PAS) to a common frame. If the site is not a quadrupole, a
     value of zero is set instead. */
...
struct __coupling_struct {
  ...
  /* Pointer to an array of the site indexes for each coupled pair within a spin
   * system, incremented in stride of 2/pair. The array size is
   * 2*number_of_couplings. */
  int *site_index;

  /* Pointer to an array of isotropic J-couplings in Hz for each pair within the
   * spin system. */
  double *isotropic_j_in_Hz;

  /* Pointer to an array of J anisotropies (zeta) in Hz for each pair within the
   * spin system. */
  double *j_symmetric_zeta_in_Hz;

  /* Pointer to an array of J asymmetry parameters (eta) for each pair within
   * the spin system. */
  double *j_symmetric_eta;

  /* Pointer to an array of symmetric J tensor Euler angles in radians, packed
     as alpha, beta, and gamma. The array size is 3*number_of_couplings. The
     Euler angle rotate the J tensor from the principal axis system (PAS) to a
     common frame. */
  ...
  /* Pointer to an array of direct-dipolar tensor Euler angles in radians,
     packed as alpha, beta, and gamma. The array size is 3*number_of_couplings.
     The Euler angle rotate the tensor from the principal axis system (PAS) to a
     common frame. */
