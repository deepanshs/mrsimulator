//
//  spatial_orientation_function.h
//
...
/**
 * The spatial orientation functions from the first-order nuclear shielding
 ...
 *
 * @param R_0 A pointer to an array of length 1 where the contribution from
 *      the zeroth-rank irreducible tensor, @f$\mathcal{R}_{0,0}^{(\sigma)}@f$,
 *      will be stored.
 * @param R_2 A pointer to an array of length 5 where the contributions from
 *      the second-rank irreducible tensor, @f$\mathcal{R}_{2,n}^{(\sigma)}@f$,
 *      will be stored ordered according to
 *      @f$\left[\mathcal{R}_{2,n}^{(\sigma)}\right]_{n=-2}^2@f$.
...
/**
 * The spatial orientation functions from the first-order electric quadrupole
 * Hamiltonian in the principal axis system, (PAS), includes the second-rank
 * irreducible tensor, given as
 * @f[ \left.
 *      \begin{array}{rl}
 ...
 *     \right\} \text{Rank-2},
 * @f]
 * where @f$\nu_q = \frac{3 C_q}{2I(2I-1)}@f$ is the quadrupole splitting
 ...
 *
 * @param R_2 A pointer to an array of length 5 where the contributions from
 *      the second-rank irreducible tensor, @f$\mathcal{R}_{2,n}^{(q)}@f$, will
 *      be stored ordered according to
 *      @f$\left[\mathcal{R}_{2,n}^{(q)}\right]_{n=-2}^2@f$.
...
/**
 * The spatial orientation functions from the second-order electric quadrupole
 * Hamiltonian, in the principal axis system (PAS), includes the zeroth,
 * second and fourth-rank irreducible tensors, given as
 * @f[\left.
 ...
 * constant.
 *
 * @param R_0 A pointer to an array of length 1 where the contribution from the
 *      zeroth-rank irreducible tensor, @f$\mathcal{R}_{0,0}^{(qq)}@f$, will be
 *      stored.
 * @param R_2 A pointer to an array of length 5 where the contributions from
 *      the second-rank irreducible tensor, @f$\mathcal{R}_{2,n}^{(qq)}@f$,
 *      will be stored ordered according to
 *      @f$\left[\mathcal{R}_{2,n}^{(qq)}\right]_{n=-2}^2@f$.
 * @param R_4 A pointer to an array of length 9 where the contributions from
 *      the fourth-rank irreducible tensor, @f$\mathcal{R}_{4,n}^{(qq)}@f$,
 *      will be stored ordered according to
 *      @f$\left[\mathcal{R}_{4,n}^{(qq)}\right]_{n=-4}^4@f$.
 * @param spin The spin quantum number, @f$I@f$.
 * @param Cq_in_Hz The quadrupole coupling constant, @f$C_q@f$, in Hz.
 * @param eta The quadrupole asymmetry parameter, @f$\eta_q \in [0, 1]@f$.
