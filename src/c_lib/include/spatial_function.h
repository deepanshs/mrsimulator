 ...
 * @f[ \left.
 *        \mathcal{R}_{0,0} = \sigma_\text{iso}
 *      \right\} \text{Rank-0},
 * @f]
 * @f[ \left.
 *      \begin{array}{r l}
 *      \mathcal{R}_{2,0} &= \zeta_\sigma, \\
 *      \mathcal{R}_{2,\pm1} &= 0, \\
 *      \mathcal{R}_{2,\pm2} &= -\frac{1}{\sqrt{6}} \eta_\sigma \zeta_\sigma,
 *      \end{array}
 *     \right\} \text{Rank-2},
 * @f]
 ...
 *
 * @param R_0 A pointer to the array of size 1 where the spatial contribution
 *      from the zeroth-rank irreducible tensor, in PAS, is stored.
 * @param R_2 A pointer to the array of size 5 where the spatial contribution
...
static inline void spatial_tensors_from_1st_order_nuclear_shielding_Hamiltonian(
    double *restrict R_0, void *restrict R_2, const double iso,
...
spatial_tensors_from_1st_order_electric_quadrupole_Hamiltonian(
    void *restrict R_2, const double spin, const double Cq, const double eta) {
 ...
 * @param R_0 A pointer to the array of size 1 where the spatial contribution
 *      from the zeroth-rank irreducible tensor, in PAS, is stored.
 * @param R_2 A pointer to the array of size 5 where the spatial contributions
 *      from the second-rank irreducible tensor, in PAS, is stored in order
 *      @f$\left[\mathcal{R}_{2,m}\right]_{m=-2}^2@f$.
 * @param R_4 A pointer to the array of size 9 where the spatial contributions
...
spatial_tensors_from_2nd_order_electric_quadrupole_Hamiltonian(
    ...
    const double spin, const double Cq, const double eta, const double vo) {
