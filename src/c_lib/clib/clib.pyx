...
def histogram2d(
    ...
    cdef ndarray[double, ndim=1] hist = np.zeros((x_count * y_count), dtype=np.float64)
    cdef int sample_count = sample_x.size
    cdef int c_long = ctypes.sizeof(ctypes.c_long)
    cdef int stride_x = sample_x.ctypes.strides[0] / c_long
    ...
    clib.histogram2d_c(
        x_count, x_min, x_max, y_count, y_min, y_max, &hist[0], sample_count,
        &sample_x[0], stride_x, &sample_y[0], stride_y, &weights[0], stride_w)
    ...
    return bins_x, bins_y, hist.reshape(x_count, y_count)
