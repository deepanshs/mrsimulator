...
x = np.linspace(0, 12, num=36)
y = np.linspace(0, 12, num=36)
pos = (x, y)

# Call the kernel generation function with the correct arguments
kernel = generate_lineshape_kernel(
    pos=pos, mth=mth, polar=True, tensor_type="quadrupolar"
)
...
# %%
# Create a set of random noise tensors
# ------------------------------------
# Below we sample a set of 200,000 random noise tensors to use during the fit; this set
# of tensors remain the same throughout the least-squares minimization making the fit
# more stable.
tensors = _czjzek_random_distribution_tensors(sigma=1, n=200_000)
...
tensor = {"Cq": 3, "eta": 0.3}
ext_cz_model = ExtCzjzekDistribution(symmetric_tensor=tensor, eps=1.6)
...
addtl_sf_kwargs = dict(
    ...
    tensors=tensors,
    n_dist=1,
    larmor_freq_Hz=mth.channels[0].larmor_freq(B0=mth.magnetic_flux_density),
    processor=processor,
    tensor_type="quadrupolar",
)
...
bestfit_model = ExtCzjzekDistribution(
    symmetric_tensor=tensor, eps=epsilon, polar=addtl_sf_kwargs["polar"]
)

xx, yy, amp = bestfit_model.pdf(pos=pos, size=10000000)

plt.figure(figsize=(5, 5))
plt.contourf(xx, yy, amp)
plt.xlabel("x / (ppm)")
plt.ylabel("y / (ppm)")
plt.tight_layout()
plt.show()
