...
MRS_plan *MRS_create_plan(unsigned int geodesic_polyhedron_frequency,
  ...
  vmz_mul(n_orientations, &plan->exp_Im_alpha[s_3], &plan->exp_Im_alpha[s_3],
          &plan->exp_Im_alpha[s_2]);

  if (allow_fourth_rank) {
    vmz_mul(n_orientations, &plan->exp_Im_alpha[s_2], &plan->exp_Im_alpha[s_3],
            &plan->exp_Im_alpha[s_1]);
    vmz_mul(n_orientations, &plan->exp_Im_alpha[s_1], &plan->exp_Im_alpha[s_3],
...
void MRS_get_amplitudes_from_plan(MRS_plan *plan, complex128 *R2,
  ...
  vm_zexp(plan->size, plan->vector, plan->vector);
  ...
  vmd_sqr(2 * plan->size, (double *)plan->vector, (double *)plan->vector);
 ...
 */
void __get_components(int number_of_sidebands,
                      double sample_rotation_frequency_in_Hz,
