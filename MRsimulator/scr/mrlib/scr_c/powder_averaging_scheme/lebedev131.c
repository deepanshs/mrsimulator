...
//Subroutine fills up arrays with alpha, beta angle pairs and weights to be used in quadrature
void Lebedev_quad_arrays(int N, double alpha[], double beta[], double qweights[]) {
    double x[N],y[N],z[N];

    Lebedev_Laikov_sphere(N, x, y, z, qweights);  //Because of the curious way it handles pointers, just one call should fill arrays
    getangle(N, x, y, z, alpha, beta);            //Similarly, loop is handled in getangle function; fills alpha and beta arrays
...
void getangle(int N, double *x, double *y, double *z, double *a, double *b) {
    ...
    for (int i=0; i<N; i++) {
        double ang_x, r;
        b[i]=acos(z[i]);                        //Two Angle integration: beta is equavalent to theta
        
        r=sqrt(x[i]*x[i]+y[i]*y[i]);
        if(r > 0) ang_x = acos(x[i]/r);
        else ang_x = acos(x[i]);
        
        if(y < 0) ang_x = -ang_x;
        a[i]=ang_x;                             //alpha is equivalent to phi
    }
}
